...
- B「Sovereign Compact（主権者の盟約）」を正式ロゴとして採用し、開始画面へ表示する。
- AとCはキービジュアル・章扉など将来用途の比較履歴として保持する。
...
## B｜Sovereign Compact（主権者の盟約）— 正式採用
...
Bを正式採用する。作品の二つの終局を損なわず、開始画面での視認性、現行UIとの色調、国家・契約・海の三要素の均衡が最もよい。現段階では生成PNGを正式ロゴとして開始画面へ組み込み、将来の最終入稿では文字輪郭、カーニング、円環の線幅をSVGで再作図し、横長・小型・単色の三ロックアップを用意する。
