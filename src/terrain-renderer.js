...
function reliefHeightCells(world, cellSize) {
  return world.tiles.filter(isLand).map((tile) => {
    const height = Math.round(clamp(0.12 + Math.max(0, tile.elevation) * 0.88) * 255);
    return `<rect x="${tile.x * cellSize}" y="${tile.y * cellSize}" width="${cellSize + 0.8}" height="${cellSize + 0.8}" fill="rgb(${height} ${height} ${height})"/>`;
  }).join("");
}
...
export function renderTerrainSvg(world, options = {}) {
  ...
  <defs>
    ...
    <filter id="terrainRelief" x="-4%" y="-6%" width="108%" height="112%">
      <feGaussianBlur stdDeviation="${(cellSize * 0.44).toFixed(2)}" result="heightField"/>
      <feDiffuseLighting in="heightField" surfaceScale="${(cellSize * 0.52).toFixed(2)}" diffuseConstant="0.72" lighting-color="#f4e9ca"><feDistantLight azimuth="315" elevation="48"/></feDiffuseLighting>
    </filter>
  ...
  <g filter="url(#terrainRelief)" mask="url(#landMask)" opacity="0.38" style="mix-blend-mode:soft-light">${reliefHeightCells(world, cellSize)}</g>
